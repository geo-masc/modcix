...
joined_cuts %>%
  ...
  geom_point(mapping = aes(shape = abs(Date_ref - Date_pred) <= 12,
  ...
  ggthemes::theme_few() +
  coord_equal(xlim = c(dates.predicted.stat %>% min(),
                       dates.predicted.stat %>% max()),
              ylim = c(dates.predicted.stat %>% min(),
                       dates.predicted.stat %>% max())) +
...
nmows <- pred_count %>% 
  ...
  lapply(function(x) {
    left_join(ref_count, x, by = c("MOD_ID", "Year", "Region")) %>% 
      replace_na(list(Group = x$Group[1],
                      Method = x$Method[1],
                      Data = x$Data[1],
                      nmow_pred = 0))
  }) %>% 
